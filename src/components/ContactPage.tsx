...
export function ContactPage() {
    ...
    return (
      <div style={{ minHeight: '100vh', background: BG, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4, ease: EASE }}
          ...
          <div style={{ width: 72, height: 72, borderRadius: '50%', background: `linear-gradient(135deg,${TEAL},#3b82f6)`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px' }}>
            <CheckCircle2 size={36} color="#0D0D12" />
          </div>
          <h1 style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: 28, fontWeight: 700, marginBottom: 12, color: TEAL }}>Message sent!</h1>
          <p style={{ color: '#9ca3af', fontSize: 16, marginBottom: 32 }}>Thanks for reaching out. We'll get back to you within one business day.</p>
          <a href="/" style={{ display: 'inline-block', background: TEAL, color: '#0D0D12', padding: '14px 32px', borderRadius: 9999, fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700, fontSize: 15, textDecoration: 'none' }}>Return to Home</a>
        </motion.div>
      </div>
    )
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, color: '#dce4e2', fontFamily: "'Hanken Grotesk',sans-serif" }}>
      <PageNav />
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: 'clamp(100px,12vw,140px) clamp(20px,5vw,40px) 80px' }}>

        {/* Header */}
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, ease: EASE }} style={{ textAlign: 'center', marginBottom: 56 }}>
          <span style={{ display: 'inline-block', fontFamily: "'Space Grotesk',sans-serif", fontSize: 12, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: TEAL, background: 'rgba(72,207,203,0.08)', border: '1px solid rgba(72,207,203,0.2)', padding: '6px 16px', borderRadius: 9999, marginBottom: 20 }}>
            Get in Touch
          </span>
          <h1 style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: 'clamp(2rem,5.5vw,3.2rem)', fontWeight: 700, lineHeight: 1.1, marginBottom: 16 }}>
            Let's Start a <span style={{ color: TEAL }}>Conversation</span>
          </h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 40, alignItems: 'start' }}>
          ...
          <motion.div initial={{ opacity: 0, x: -24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.65, ease: EASE, delay: 0.1 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24, marginBottom: 36 }}>
              {[
                ...
                <div key={label} style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: `linear-gradient(135deg,${TEAL},#3b82f6)`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    {icon}
                  </div>
                  <div>
            ...
            <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 20, padding: 24 }}>
              ...
              <p style={{ fontSize: 14, color: '#9ca3af', marginBottom: 20, lineHeight: 1.6 }}>Skip the form and book a discovery call directly.</p>
              <a href="/book" style={{ display: 'block', textAlign: 'center', background: TEAL, color: '#0D0D12', padding: '13px 0', borderRadius: 9999, fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700, fontSize: 15, textDecoration: 'none', transition: 'transform 0.2s' }}
          ...
          <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.65, ease: EASE, delay: 0.2 }}
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 13, color: '#9ca3af', marginBottom: 8, fontWeight: 500 }}>Your Name <span style={{ color: '#ef4444' }}>*</span></label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 13, color: '#9ca3af', marginBottom: 8, fontWeight: 500 }}>Subject</label>
                <select value={form.subject} onChange={e => set('subject', e.target.value)}
                  style={{ ...inputStyle, appearance: 'none' as const }}>
                  <option value="" style={{ background: '#12121A' }}>What's this about?</option>
                  {SUBJECTS.map(s => <option key={s.value} value={s.value} style={{ background: '#12121A' }}>{s.label}</option>)}
                </select>
              </div>

              <div>
                <label style={{ display: 'block', fontSize: 13, color: '#9ca3af', marginBottom: 8, fontWeight: 500 }}>Your Message <span style={{ color: '#ef4444' }}>*</span></label>
              ...
              {/* Terms */}
              <div style={{ padding: '14px 18px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.07)', background: 'rgba(0,0,0,0.15)' }}>
              ...
              <button type="submit" disabled={isSubmitting}
                style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 10, background: TEAL, color: '#0D0D12', padding: '15px 32px', borderRadius: 9999, border: 'none', cursor: isSubmitting ? 'not-allowed' : 'pointer', fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700, fontSize: 16, opacity: isSubmitting ? 0.7 : 1, transition: 'transform 0.2s, opacity 0.2s', width: '100%' }}
              ...
              {/* Disclaimer */}
              <div style={{ fontSize: 12, color: 'rgba(156,163,175,0.65)', lineHeight: 1.6 }}>
